...
// This dashboard is deployed as a normal web app, not an offline-first PWA.
// Remove service workers left by older Flutter builds before loading the app;
// otherwise they can keep serving an obsolete index.html/main.dart.js forever.
(async () => {
  const wasControlledByServiceWorker =
  ...
  // unregister() does not release the page currently controlled by the old
  // worker. Reload once so all app files come directly from the network.
  if (wasControlledByServiceWorker &&
      sessionStorage.getItem('flutter-sw-cleanup-reload') !== 'done') {
    sessionStorage.setItem('flutter-sw-cleanup-reload', 'done');
    window.location.reload();
    return;
  }
  sessionStorage.removeItem('flutter-sw-cleanup-reload');

  // Deliberately omit serviceWorkerSettings so Flutter does not register one.
